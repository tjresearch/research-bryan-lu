...
def rotation_about_point(p0, center, theta):
    vector = p0.x - center.x, p0.y - center.y
    radian = theta * np.pi / 180
    rotated = vector[0] * np.cos(radian) - vector[1] * np.sin(radian), vector[0] * np.sin(radian) + vector[1] * np.cos(radian)
    image = rotated[0]+center.x, rotated[1]+center.y
    return instantiators['point'](image[0], image[1])
