...
from geosolver.diagram.computational_geometry import distance_between_line_and_point, line_length, \
    ...
    midpoint, rotation_about_point, intersections_between_lines
...
def Collinear(A, B, C): # fix spelling
...
def Circumcenter(triangle):
    m = midpoint(triangle.a, triangle.b)
    n = midpoint(triangle.b, triangle.c)
    p = rotation_about_point(triangle.a, m, 90)
    q = rotation_about_point(triangle.b, n, 90)
    return intersections_between_lines(Line(m, p), Line(n, q), 1)  # FIX?


def Circumradius(triangle):
    return distance_between_points(triangle.a, Circumcenter(triangle))


def Circumcircle(triangle):
    return Circle(Circumcenter(triangle), Circumradius(triangle))
...
def Inradius(triangle):
    lenA = distance_between_points(triangle.b, triangle.c)
    lenB = distance_between_points(triangle.c, triangle.a)
    lenC = distance_between_points(triangle.a, triangle.b)
    s = (lenA + lenB + lenC)/2
    area = np.sqrt(s*(s-lenA)*(s-lenB)*(s-lenC))
    return area/s


def Incircle(triangle):