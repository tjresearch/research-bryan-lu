...
class StanfordDependencyParser(SyntaxParser):
    ...
    def get_syntax_parses(self, words, k, unique=True, parser=True):
        ...
        words = {key: word.lstrip().rstrip() for key, word in words.items()} # words should be index(number): word
        if not parser:
        ...
        data = nlp.annotate(" ".join(neutral_sentence),
                   properties={
                       'annotators': "depparse",
                       'outputFormat': 'json',
                       'timeout': 1000,
        ...
        #params = {'properties' : {"annotators" : "tokenize,ssplit,pos"}, 'outputFormat' : 'json', 'data' : ' '.join(neutral_sentence)}
        #print(requests.post('http://localhost:9000/?properties={"annotators":"tokenize,ssplit,pos",
        # "outputFormat":"json"}', data = {'data':'The quick brown fox jumped over the lazy dog.'}).text)
        #r = requests.post('http://localhost:9000/?properties={"annotators":"tokenize,ssplit,pos,depparse","outputFormat":"json"}',
        #            data={'data':' '.join(neutral_sentence)})
        #data = r.json()
        trees = []
        ##
        ...
        #
        #
        for rank, tree_data in enumerate(data.values()): # tree_data is a dictionary with keys 'score' and 'tuples'
